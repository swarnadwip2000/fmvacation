...
use App\Mail\SendCodeResetPasswordUser;
...
class ForgotPasswordController extends Controller
{
    ...
    public function forgetPasswordCheck(Request $request)
    {
        ...
        $count = User::where('email', $request->email)->role('USER')->count();
        if ($count > 0) {
            $user = User::where('email', $request->email)->select('id', 'first_name','last_name', 'email')->first();
            ...
            Mail::to($request->email)->send(new SendCodeResetPasswordUser($details));
    ...
    public function resetPassword($id, $token)
    {
        ...
        if ($newTime > date('h:i A')) {
            return view('frontend.auth.reset-password', compact('user', 'token','id'));
        } else {
